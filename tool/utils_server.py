...
def save_network(network, dirname, epoch_label):
    ...
    # 保存到checkpoints目录
    checkpoint_save_path = os.path.join(target_dir, save_filename)
    torch.save(network.cpu().state_dict(), checkpoint_save_path)
    
    # 保存到results目录
    results_save_path = os.path.join(results_dir, save_filename)
    torch.save(network.cpu().state_dict(), results_save_path)
