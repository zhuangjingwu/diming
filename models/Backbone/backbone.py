...
class Backbone(nn.Module):
    ...
    def init_backbone(self, img_size):
        ...
        if backbone == "ResNet50":
            ...
            # 获取Swin-T的具体配置参数
            patch_size = self.opt.model["backbone"].get("patch_size", 4)
            window_size = self.opt.model["backbone"].get("window_size", 7)
            
            # 构建timm模型名称
            model_name = f"swin_tiny_patch{patch_size}_window{window_size}_224"
            
            backbone_model = timm.create_model(
                model_name, pretrained=pretrain, img_size=img_size, features_only=True)
            
            # 根据patch_size确定输出通道
            if patch_size == 16:
                backbone_out_channel = [384, 768, 1536, 3072]
            else:  # 默认patch_size=4
                backbone_out_channel = [96, 192, 384, 768]
    ...
    def forward(self, image, image_extra=None):
        ...
        if self.backbone_type in ["DeiT-S"]:
            features = self.backbone.forward_features(image)[:, 1:]
            features = [vector2array(features)]
        elif self.backbone_type in ["ResNet50", "EfficientNet-B5", "Swin-T"]:
            # 使用features_only=True的模型直接调用forward
