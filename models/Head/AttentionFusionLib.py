...
class AttentionFusionLib(nn.Module):
    ...
    def forward_single(
        ...
        if self.fusion_type == "CAT_SA":
            # 保存原始空间尺寸
            temp_h, temp_w = src_temp.shape[2], src_temp.shape[3]
            search_h, search_w = src_search.shape[2], src_search.shape[3]
            ...
            if self.enable_position_embedding:
                # 动态调整位置编码大小以匹配实际特征
                pos_embed_uav_resized = F.interpolate(
                    self.pos_embed_uav.transpose(1, 2).reshape(1, self.d_model, self.grid_size_uav, self.grid_size_uav),
                    size=(temp_h, temp_w),
                    mode='bilinear',
                    align_corners=False
                ).flatten(2).transpose(1, 2)
                
                pos_embed_satellite_resized = F.interpolate(
                    self.pos_embed_satellite.transpose(1, 2).reshape(1, self.d_model, self.grid_size_satellite, self.grid_size_satellite),
                    size=(search_h, search_w),
                    mode='bilinear',
                    align_corners=False
                ).flatten(2).transpose(1, 2)
                
                pos_search = torch.cat((pos_embed_uav_resized, pos_embed_satellite_resized), dim=1).transpose(0, 1)

        elif self.fusion_type == "CA":
            # 保存原始形状
            src_temp_original_shape = src_temp.shape
            src_search_original_shape = src_search.shape
            
            # 转换为序列格式
            src_temp = src_temp.flatten(2).permute(2, 0, 1)  # (H*W)×B×C
            src_search = src_search.flatten(2).permute(2, 0, 1)  # (H*W)×B×C

            if self.enable_position_embedding:
                # 动态调整位置编码大小以匹配实际特征
                pos_embed_uav_resized = F.interpolate(
                    self.pos_embed_uav.transpose(1, 2).reshape(1, self.d_model, self.grid_size_uav, self.grid_size_uav),
                    size=(src_temp_original_shape[2], src_temp_original_shape[3]),
                    mode='bilinear',
                    align_corners=False
                ...
                pos_embed_satellite_resized = F.interpolate(
                    self.pos_embed_satellite.transpose(1, 2).reshape(1, self.d_model, self.grid_size_satellite, self.grid_size_satellite),
                    size=(src_search_original_shape[2], src_search_original_shape[3]),
                    mode='bilinear',
                    align_corners=False
                ).flatten(2).transpose(1, 2).transpose(0, 1)
                
                pos_temp = pos_embed_uav_resized
                pos_search = pos_embed_satellite_resized
        ...
        if self.fusion_type == "CAT_SA":
            ...
            # 对于CA模式，我们修改了src1（UAV特征）
            memory_temp = memory_temp.permute(1, 2, 0).contiguous()  # B×C×(H*W)
            memory_search = memory_search.permute(1, 2, 0).contiguous()  # B×C×(H*W)
            
            # 恢复UAV特征的空间形状
            memory_temp = memory_temp.view(memory_temp.shape[0], memory_temp.shape[1], temp_size, temp_size)  # B×C×H×W
            
            # 恢复卫星特征的空间形状
            memory_search = memory_search.view(memory_search.shape[0], memory_search.shape[1], search_size, search_size)  # B×C×H×W

        # 在CA模式下，我们需要使用修改后的UAV特征来寻找卫星图像中的匹配区域
        if self.fusion_type == "CA":
            # 使用修改后的UAV特征与原始卫星特征进行相关操作
            # 这里我们可以直接返回修改后的UAV特征，或者使用它来生成匹配热图
            # 为了保持与原始代码的一致性，我们仍然处理memory_search
            pass
        else:
            # 应用池化并恢复最终形状
        ...
        # 确保输出形状正确
        memory_search = memory_search.view(
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward_post(self, src1, src2,
        ...
        if self.fusion_type == "CA":  # src1 is UAV, src2 is satellite
            # UAV token作为Query，卫星feature map作为Key/Value
            query = self.with_pos_embed(src1, pos_src1)  # UAV特征作为查询
            key = self.with_pos_embed(src2, pos_src2)    # 卫星特征作为键
            value = src2                                 # 卫星特征作为值
            
            # 使用UAV特征查询卫星特征
            src1 = self.multihead_attn1(query=query, key=key, value=value, 
                                         attn_mask=src2_mask,
                                         key_padding_mask=src2_key_padding_mask)[0]

            # 保持与原始代码一致的更新方式
            src1 = src1 + self.dropout11(src1)
